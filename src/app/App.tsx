...
function Container({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`max-w-[1680px] mx-auto px-[80px] min-[1440px]:px-[160px] w-full ${className}`}>
      {children}
    </div>
  );
}
...
function PhoneShot({ src }: { src: string }) {
  return (
    <div className="flex items-center p-[8px] shrink-0">
      <div className="h-[410px] w-[192px] min-[1440px]:h-[510px] min-[1440px]:w-[236px] relative rounded-[24px] shrink-0">
...
function PhonePair({ src1, src2, reverse = false }: { src1: string; src2: string; reverse?: boolean }) {
  ...
  return (
    <div className="flex-1 min-w-0">
      ...
      <div className={`hidden min-[1440px]:flex gap-[40px] items-center ${reverse ? 'justify-end' : ''}`}>
        <PhoneShot src={src1} />
        <PhoneShot src={src2} />
      </div>
      {/* Tablet: cascade — phone1 front, phone2 behind+smaller with shadow */}
      <div className={`flex min-[1440px]:hidden items-center h-[426px] ${reverse ? 'justify-end' : ''}`}>
        {/* Front phone */}
...
function HeroSection() {
  ...
  useEffect(() => {
    const update = () => {
      const tablet = window.innerWidth < 1440;
      setIsTablet(tablet);
      if (tablet) {
        setPhoneScale(0.9);
      } else {
        const sectionH = Math.min(window.innerHeight - 72, 800);
  ...
  const sectionStyle = isTablet
    ? { minHeight: '720px' }
    : { height: 'calc(100vh - 72px)', minHeight: '720px', maxHeight: '800px' };
  ...
  return (
    <section id="hero" className="bg-[#fef7ff] relative flex items-center" style={sectionStyle}>
      ...
      {/* Content */}
      <Container className={`relative z-10 flex flex-col min-[1440px]:flex-row items-center justify-center py-[72px] min-[1440px]:py-0${isTablet ? ' gap-[56px]' : ' gap-[40px]'}`}>
        {/* Left / top column */}
        <div className="flex flex-col gap-[32px] items-center min-[1440px]:items-start w-full min-[1440px]:flex-1 min-[1440px]:min-w-0">
          <div className="flex flex-col gap-[32px] items-center min-[1440px]:items-start w-full">
            <h1 className="font-['Roboto_Condensed',sans-serif] font-bold text-[#4f378a] text-[64px] leading-[72px] tracking-[-0.25px] text-center min-[1440px]:text-left">
              Литовский язык&nbsp;для русскоязычных
            </h1>
            <div className="flex flex-col gap-[16px] items-center min-[1440px]:items-start w-full">
              <p className="font-['Roboto',sans-serif] font-normal text-[#4a4459] text-[24px] leading-[32px] text-center min-[1440px]:text-left">
                Долгожданное мобильное приложение для системного изучения литовского языка:
              </p>
              {/* Feature list — desktop only (inside left col) */}
              <div className="hidden min-[1440px]:flex flex-col gap-[12px] w-full">
                {FEATURE_ITEMS.map((t) => (
          ...
          <div className="flex gap-[24px] items-start justify-center min-[1440px]:justify-start">
        ...
        {/* Phone mockup */}
        <div className={`flex justify-center min-[1440px]:flex-1 min-[1440px]:min-w-0 min-[1440px]:items-center relative z-[2]${isTablet ? ' w-full items-start' : ''}`}
        ...
        <div className="block min-[1440px]:hidden w-full bg-[#6750a4] rounded-[24px] overflow-hidden relative z-0">
          {/* blur blob inside */}
...
// ─── Feature sections ─────────────────────────────────────────────────────────
function FeatureRow({ left, right, bg = "white" }: { left: React.ReactNode; right: React.ReactNode; bg?: string }) {
  return (
...
function Section3() {
  return (
    <div className="relative bg-[#fef7ff] overflow-hidden w-full">
      ...
      <div className="absolute right-[191px] top-[438px] size-[50px] rotate-75 pointer-events-none">
        <div className="absolute inset-[8.28%_8.33%_8.44%_8.44%]" style={{ maskImage: `url("${imgIcon}")`, maskRepeat: "no-repeat", maskSize: "24px 24px" }}>
          <svg className="absolute block inset-0 size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 41.4582 41.4847">
            <path d={svgPaths.p17507300} fill="#EADDFF" />
          </svg>
        </div>
      </div>
    </div>
    ...
    <div className="relative overflow-hidden w-full bg-white">
      <Container className="flex gap-[40px] items-center py-[80px]">
        <SectionText
          ...
          items={["визуальные образы", "произношение слов", "примеры использования", "дополнительные пояснения", "литовская озвучка", "сохранение нужных слов в словарь"]}
        />
        <div className="relative flex-1 min-w-0">
          <PhonePair src1={imgScreenshot6} src2={imgScreenshot7} reverse />
          {/* Avatar anchored to phone block */}
          <div className="absolute rounded-full size-[120px] overflow-hidden pointer-events-none z-[3]"
    ...
    <footer className="bg-[#e8def8] py-[28px] w-full">
      <Container className="flex items-center justify-between">
        ...
        <div className="flex gap-[40px] items-center font-['Roboto',sans-serif] font-medium text-[#625b71] text-[14px] leading-[20px] tracking-[0.1px]">
          <a href="#" className="hover:text-[#4f378a] transition-colors whitespace-nowrap">Privacy Policy</a>
          <a href="#" className="hover:text-[#4f378a] transition-colors whitespace-nowrap">Terms of Use</a>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white font-['Roboto',sans-serif]">
      {/* ── Sticky Header ── */}
      <header
        className="fixed top-0 left-0 right-0 z-50 bg-white transition-all duration-300 border-b border-[#cac4d0]"
      ...
      >
        <Container className="flex items-center justify-between">
          ...
          <nav className="flex gap-[40px] items-center font-['Roboto',sans-serif] font-medium text-[#625b71] text-[14px] leading-[20px] tracking-[0.1px]">
            <button onClick={() => scrollTo("hero")} className="hover:text-[#4f378a] transition-colors cursor-pointer bg-transparent border-none font-medium text-[14px] tracking-[0.1px]">В начало</button>
            <button onClick={() => scrollTo("features")} className="hover:text-[#4f378a] transition-colors cursor-pointer bg-transparent border-none font-medium text-[14px] tracking-[0.1px]">Преимущества</button>
            <button onClick={() => scrollTo("download")} className="hover:text-[#4f378a] transition-colors cursor-pointer bg-transparent border-none font-medium text-[14px] tracking-[0.1px]">Скачать</button>
            <button onClick={() => scrollTo("contact")} className="hover:text-[#4f378a] transition-colors cursor-pointer bg-transparent border-none font-medium text-[14px] tracking-[0.1px]">Обратная связь</button>
          </nav>
        </Container>
      </header>
      ...
      <div
        ...
        style={{ bottom: `${buttonBottom}px`, opacity: showScrollTop ? 1 : 0, pointerEvents: showScrollTop ? "auto" : "none" }}
      >
        <button
          onClick={() => scrollTo("hero")}
        ...
        >
          <div className="size-[24px] relative">
            <div
              ...
              style={{ maskImage: `url("${imgArrowUpward}")`, maskRepeat: "no-repeat", maskPosition: "-4px -4px", maskSize: "24px 24px" }}
            >
